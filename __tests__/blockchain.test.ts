...
describe('blockchain integration', () => {
  // These tests require network access - skip in CI unless INTEGRATION_TESTS=true
  const runIntegration = process.env.INTEGRATION_TESTS === 'true'

  describe.skip('getBalance (requires INTEGRATION_TESTS=true)', () => {
    it('fetches balance from mainnet', async () => {
      if (!runIntegration) return
      const { getBalance } = await import('../src/blockchain')

      // Vitalik's address (should always have some ETH)
      const address = '0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045'
      const balance = await getBalance(address, 'https://eth.llamarpc.com')

      expect(balance).toBeGreaterThan(0n)
    })
  })

  describe.skip('checkCanaryBalance (requires INTEGRATION_TESTS=true)', () => {
    it('checks canary balance', async () => {
      if (!runIntegration) return
      const { checkCanaryBalance } = await import('../src/blockchain')

      const result = await checkCanaryBalance(
